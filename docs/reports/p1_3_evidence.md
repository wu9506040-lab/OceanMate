...
> **关键 KPI**：拿到完整链所需尝试次数 **4/20**（PDA confidence ≥ 0.7 时即触发链式；不同 query 触发率因 LLM 随机性波动）
>
> ⚠️ **数据诚实声明**：此前的"1/20 首次命中"为乐观表述；本次核查（2026-08-14）实测 4/20。完整链路在 PDA confidence ≥ 0.7 时稳定触发，**4/20 = 20% 的 query 能拿到完整链**（其余因 LLM confidence 不足 0.7 未触发），非"100% 必然触发"。
