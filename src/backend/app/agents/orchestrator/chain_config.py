...
# 链路 1：PDA → TRA（Day 18 P1-final 禁用）
...
# Day 18 P1-final：禁用 PDA → TRA 自动派单链式触发。
# 产品逻辑：bot 能答的问题（PDA 高置信度）应自助给方案，**不自动派单**。
#   商户主动说"需要运营团队介入/跟进/处理" → 走 ticket_routing 关键词分类，单独派单。
#   这样避免「机器人答得了却写简报找人工」的逻辑矛盾。
#   保留 chain_rule 定义（注释形式），方便未来有需要时重新启用。
# PDA_TO_TRA_CHAIN = {
...
#     },
# }
# Day 18 P1-final：用空 rule 替代（让 Orchestrator chain 逻辑仍能找到 rule 但 trigger 永远 False）
PDA_TO_TRA_CHAIN = {
    "next_tool": "ticket_routing",
    "trigger": lambda prev_data: False,  # 永远不触发
    "params_builder": lambda prev_data, ctx: {},
}
