...
# Day 18 P2-final：审核命令已从聊天移除 — 运营/lead 直接在飞书多维表格 review_decisions
# 表改 decision 列审核（pending_review → approved / rejected）。后端通过
# /admin/sync-bit 或飞书多维表格 webhook（生产预留）反向同步决策。
#
# 保留 _KEA_CASE_ID_RE 给 _list_review_history 等其他场景共用。
...
def route_kea(query: str, ctx: dict, matched: list[str], registry: ToolRegistry) -> dict:
    ...
    Day 18 P2-final：审核命令（approve_case / reject_case）已从聊天移除 — 运营/lead
    直接在飞书多维表格 review_decisions 表改 decision 列。后端通过
    /admin/sync-bit endpoint 或飞书多维表格 webhook 反向同步决策。
    approve_case / reject_case 仍保留为内部 API（被反向同步 handler 调用）。
    """
...
def _detect_review_command(query: str) -> tuple[Optional[str], Optional[str]]:
    """检测人工审核命令（Day 18 P2-final 已废弃 — 审核全在多维表格）。

    历史背景：Day 17 v3 数字员工闭环第 5 段，运营/lead 通过聊天命令触发审核。
    Day 18 P2-final 改为：运营/lead 在飞书多维表格 review_decisions 表改 decision 列，
    后端通过 /admin/sync-bit 或飞书多维表格 webhook 反向同步。

    函数体保留空壳仅做 import-time 兼容（如有外部测试），实际不再被 route_kea 调用。
    """
    return None, None
